namespace CefGlue.ScriptableObject
{
    ...
    internal class ScriptableObjectBinder
    {
        ...
        private ScriptableObjectBinder(Type type, NamingConvention namingConvention)
        {
            ...
            if (!type.IsClass) throw new ArgumentException("The type must be a class.");
        ...
        private static bool IsScriptable(MemberInfo member)
        {
        ...
        private static ScriptableAttribute GetScriptableAttribute(MemberInfo member)
        {
            var attributes = member.GetCustomAttributes(typeof(ScriptableAttribute), true);
            if (attributes.Length == 0) { return null; }
            else if (attributes.Length == 1)
            {
                return (ScriptableAttribute)attributes[0];
            }
